...
if __name__ == "__main__":
	...
	if not os.path.isdir(os.path.join(main_path)):
		os.mkdir(os.path.join(main_path))

	if not os.path.isdir(os.path.join(main_path, "/files_256")):
		os.mkdir(os.path.join(main_path, "/files_256"))

	if not os.path.isdir(os.path.join(main_path, "/data")):
		os.mkdir(os.path.join(main_path, "/data"))

	if not os.path.isdir(os.path.join(main_path, "/kinect")):
		os.mkdir(os.path.join(main_path, "/kinect"))

	cls_id = np.arange(1,121)
	cls_id = list(cls_id)

	print(cls_id, len(cls_id))
	# cls_id = [1,24,51,100]
	...
	count = 0
	complete = [ 3, 19, 31,  4, 30, 23, 24, 12, 28, 13, 18, 20, 27,  5,  7,  2,  6, 8, 11,  9]
	json_path = main_path + "/NTU-VIBE/"
	...
	for i in json_file:
		...
		s = int(s[-3:])
		# if s in complete:
		# 	print('set up id {} completed..'.format(s))
		# 	continue
		...
		for file in json_data:
			...
			for people in data.keys():
				...
				bbox = torch.empty((max_t, 4))

				bbox[:t] = torch.tensor(data[people]['bboxes'])[:t]
			...
			if count%1000==0:
				print(count, file, setup_id, y, cam)
			count += 1
		...
		batch_meanpose  = batch_meanpose.view(B,T,-1,3)
		batch_bbox = batch_bbox.view(B,T,4)
		...
		print('Bounding box shape: ', batch_bbox.shape)
		labels = np.array(labels)
		print('Label shape: ', labels.shape)
		camera = np.array(camera)
		print('Camera shape:', camera.shape)
		
		if not os.path.isdir(main_path + "/files_256"):
			os.mkdir(main_path + "/files_256")
		
		print('Saving Data..')
		np.save(main_path + '/files_256/X_{}'.format(setup_id), np.array(recreated_joints))
		np.save(main_path + '/files_256/Y_{}'.format(setup_id), np.array(labels))
		np.save(main_path + '/files_256/Euler_{}'.format(setup_id), np.array(batch_angles))
		np.save(main_path + '/files_256/Bbox_{}'.format(setup_id), np.array(batch_bbox))
		np.save(main_path + '/files_256/Mean_pose_{}'.format(setup_id), np.array(batch_meanpose))
		np.save(main_path + '/files_256/Setup_list_{}'.format(setup_id), np.array(setup_list))
		np.save(main_path + '/files_256/Persion_idx_{}'.format(setup_id), np.array(person_idx))
		np.save(main_path + '/files_256/Seq_len_{}'.format(setup_id), np.array(seq_len))
		np.save(main_path + '/files_256/Rot6d_{}'.format(setup_id), np.array(rot6d))
		np.save(main_path + '/files_256/Camera_{}'.format(setup_id), np.array(camera))
		np.save(main_path + '/files_256/File_name_{}'.format(setup_id), np.array(file_name_list))
		index.append(setup_id)
		np.save(main_path + '/files_256/index', np.array(index))
		print('Data Saved')
		print('=================================================')
