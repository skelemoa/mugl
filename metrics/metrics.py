...
def calculate_mmd(sequence_1, sequence_2, bandwidth, mode='MMD-A'):  # compute the mmd between sequences, shape (N*len*dim)
...
def mmd_function(gen, real, y):
	labels = np.unique(y)
	new_r = 0
	result_list = []
	for j in range(-4,10):
		new_new_r = calculate_mmd(gen, real, 10 ** j)
		if new_new_r > new_r:
			new_r = new_new_r
